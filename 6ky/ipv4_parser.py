...
def ipv4__parser(ip_addr, mask):
    ...
    subnet_parts = mask.split('.')

    network_parts = []
    host_parts = []

    for i in range(4): 
        ip_num = int(ip_parts[i])
        subnet_num = int(subnet_parts[i])
        network_num = ip_num & subnet_num
        host_num = ip_num - network_num
        network_parts.append(str(network_num))
        host_parts.append(str(host_num))
    network_block = '.'.join(network_parts)
    host_identifier = '.'.join(host_parts)
